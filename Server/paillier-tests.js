...
const { publicKey, privateKey } = paillier.generateRandomKeys(64); // Change to at least 512 bits in production state
...
let bn1 = bigInt((-10)).mod(publicKey.n);
let zeroencrypted = publicKey.encrypt(bn1).toString()
let encryptedNumber = publicKey.encrypt(6).toString()


console.log(zeroencrypted, encryptedNumber);
console.log(privateKey.decrypt(zeroencrypted));

let sum = publicKey.addition(zeroencrypted, encryptedNumber);
console.log(sum);
let decrypedSum = privateKey.decrypt(sum);
console.log(decrypedSum);
